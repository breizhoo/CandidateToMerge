...
namespace BinaryComponents.SuperList.ItemLists
{
	...
	public abstract class ItemList
	{
		...
		/// <returns></returns>
		protected internal abstract object this[int index] { get; }
		...
		protected internal ListControl ListControl
		{
			...
			set
			{
				if( _listControl != value )
				{
					...
					{
						_listControl.Columns.DataChanged -= new EventHandler<EventingList<Column>.EventInfo>( Columns_DataChanged );
						this.DetachFromColumnsEvents( _listControl.Columns.ToArray() );
					}
					_listControl = value;
					if( _listControl != null )
					{
						_listControl.Columns.DataChanged += new EventHandler<EventingList<Column>.EventInfo>( Columns_DataChanged );
						this.AttachToColumnsEvents( _listControl.Columns.ToArray() );
					}
				}
			}
		}
		...
		{
			this.ListControl.UpdateListSection( lazyLayout );
		}

		private void Columns_DataChanged( object sender, BinaryComponents.Utility.Collections.EventingList<Column>.EventInfo e )
		{
			switch( e.EventType )
			{
		...
		private void column_DataChanged( object sender, Column.ColumnDataChangedEventArgs e )
		{
			switch( e.WhatChanged  )
			{
				...
				case Column.WhatPropertyChanged.IsVisible:
					if( e.Column.SortOrder != System.Windows.Forms.SortOrder.None )
					{
						this.Sort();
					}
					break;
