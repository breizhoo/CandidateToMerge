...
namespace BinaryComponents.SuperList
{
	...
	{
		public enum TypeOfChange { Inserted, Deleted };
	...
	public class SelectedItemsCollection: ICollection<RowIdentifier>
	{
		...
		public RowIdentifier this[int i]
		{
			get
			{
				return this.GetSortedSelectionList()[i];
			}
		}
		public void Add( object o )
		{
			this.Add( new ListSection.NonGroupRow( o ) );
		}
		...
		public void Clear()
		{
			...
			{
				RowIdentifier[] items = this.GetSortedSelectionList();
				_selectedRows.Clear();
				if( items.Length > 0 )
				{
					this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Deleted, items );
				}
			}
			else
			{
		...
		{
			foreach( RowIdentifier ri in this.GetSortedSelectionList() )
			{
				yield return ri;
			}
		}
		...
		{
			return this.GetSortedSelectionList().GetEnumerator();
		}
		...
		internal int ClearAndAdd( params RowIdentifier[] items )
		{
			...
			{
				this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Deleted, deletedItems.ToArray() );
			}
			if( insertedItems.Count > 0 )
			{
				this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Inserted, insertedItems.ToArray() );
			}
			this.ClearCache();
		...
		internal void AddInternal( RowIdentifier item )
		{
			...
			if( !_selectedRows.ContainsKey( item ) )
			{
				...
				if( pos != -1 )
				{
					...
					this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Inserted, item );
				}
			}
		}
		...
		internal void AddRangeInternal( RowIdentifier []selectedItems )
		{
			...
			else
			{
				...
				if( items.Count > 0 )
				{
					items.Sort( RowIdentifierComparision );
					this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Inserted, items.ToArray() );
				}
			}
		}

		private int RowIdentifierComparision( RowIdentifier x, RowIdentifier y )
		{
			if( x.Equals( y ) )
			{
				return 0;
			...
			//	Groups are always first compared with non groupd items.
			int groupPosX = _listControl.Columns.GroupedItems.IndexOf( x.ColumnGroup );
			int groupPosY = _listControl.Columns.GroupedItems.IndexOf( y.ColumnGroup );
		...
		internal bool RemoveInternal( RowIdentifier rowIdentifier )
		{
			...
			if( _selectedRows.ContainsKey( rowIdentifier ) )
			{
				...
				this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Deleted, rowIdentifier );
				return true;
			}
			return false;               
		}
		...
		internal void RemoveRangeInternal( RowIdentifier[] rowIdentifiers )
		{
			...
			else
			{
				...
				this.OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange.Deleted, itemsArray );
			}
		}
		...
		private void OnDataChanged( SelectedItemsChangedEventArgs.TypeOfChange changeType, params RowIdentifier[] items )
		{
			if( this.DataChanged != null )
			{
				...
				this.DataChanged( _listControl, eventArgs );
			}
		}
